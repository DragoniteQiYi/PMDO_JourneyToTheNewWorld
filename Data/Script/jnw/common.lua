...
function COMMON.SetPlayerFromData(data)
...
function COMMON.EndDungeonWithFanfare(result, zoneId, structureId, mapId, entryId) -- End a dungeon with fanfare, usually for job/general completions
	GAME:EndDungeonRun(result, zoneId, structureId, mapId, entryId, true, true)
	GAME:EnterZone(zoneId, structureId, mapId, entryId)
end

function COMMON.EndStoryDungeon(result, zoneId, structureId, mapId, entryId) -- End a dungeon without fanfare, usually used for story events
	GAME:EndDungeonRun(result, zoneId, structureId, mapId, entryId, false, true)
	GAME:EnterZone(zoneId, structureId, mapId, entryId)
end

function COMMON.SetProgress(chapter, section) -- Utility function for testing stuff
	SV.Progression.Chapter = chapter
	SV.Progression.SectionFlag = section
end

function COMMON.SetSpeakerHero()
	if SV.General.Starter == nil then SV.General.Starter = GAME:GetPlayerPartyMember(0) end 
...
function COMMON.SetSpeakerPartner()
  UI:SetSpeaker('', false, SV.General.Partner.CurrentForm.Species, SV.General.Partner.CurrentForm.Form, SV.General.Partner.CurrentForm.Skin, SV.General.Partner.CurrentForm.Gender)

end

function COMMON.SetSpeakerUnknown(char)
	if char ~= nil then
